...
import { Card, Table, Tag, Tabs, Button, Upload, Modal, message, Descriptions, List, Space, Input, Collapse, Select, Divider } from "antd";
...
interface CaseDetailData {
  ...
  assignments: Array<{ id: string; expertId: string; expertName: string; status: string; declarationSigned: boolean; assignedAt: string }>;
  ...
  auditLogs: Array<{ id: string; action: string; targetType: string; targetId: string; details: string | null; userName: string; createdAt: string }>;
  experts: Array<{ id: string; name: string; availability: string }>;
}
...
export default function CaseDetailClient({ data }: { data: CaseDetailData }) {
  ...
  const handleStatusChange = async () => {
    ...
    const result = await updateCaseStatus(data.id, selectedStatus);
  ...
  const handleAssignExpert = async () => {
    ...
    setAssigning(true);
    const result = await assignExpert(data.id, selectedExpert);
    setAssigning(false);
    if (result.error) { message.error(result.error); return; }
    message.success("专家指派成功");
  ...
  const handleAiAnalysis = async (type: string) => {
    setAiLoading(type);
    let result;
    if (type === "CLAIM_INTERPRETATION") result = await triggerClaimAnalysis(data.id);
    else if (type === "INFRINGEMENT_COMPARISON") result = await triggerInfringementComparison(data.id);
    else result = await triggerValuation(data.id);
    setAiLoading(null);
  ...
  const assignedExpertIds = new Set(data.assignments.map((a) => a.expertId));
  const availableExperts = data.experts.filter((e) => !assignedExpertIds.has(e.id));
  const allowedNext = ALLOWED_TRANSITIONS[data.status] || [];
    ...
    <div>
      ...
      {data.assignments.length > 0 && (
        <Card title="专家指派" size="small">
          <Table
            dataSource={data.assignments} rowKey="id" pagination={false}
            columns={[
              { title: "专家", dataIndex: "expertName" },
              { title: "状态", dataIndex: "status", render: (v: string) => {
                const m: Record<string, { label: string; color: string }> = { PENDING: { label: "待接受", color: "orange" }, ACCEPTED: { label: "已接受", color: "green" }, DECLINED: { label: "已拒绝", color: "red" }, COMPLETED: { label: "已完成", color: "blue" } };
                const s = m[v]; return s ? <Tag color={s.color}>{s.label}</Tag> : v;
              }},
              { title: "独立性声明", dataIndex: "declarationSigned", render: (v: boolean) => v ? <Tag color="green">已签署</Tag> : <Tag>未签署</Tag> },
              { title: "指派时间", dataIndex: "assignedAt", render: (v: string) => new Date(v).toLocaleDateString("zh-CN") },
            ]}
          />
        </Card>
      )}
  ...
  const aiContent = (
    <div>
      <div style={{ marginBottom: 16 }}>
        <Space>
          <Button type="primary" loading={aiLoading === "CLAIM_INTERPRETATION"} onClick={() => handleAiAnalysis("CLAIM_INTERPRETATION")}>权利要求解释</Button>
          <Button loading={aiLoading === "INFRINGEMENT_COMPARISON"} onClick={() => handleAiAnalysis("INFRINGEMENT_COMPARISON")}>侵权比对</Button>
          <Button loading={aiLoading === "VALUATION"} onClick={() => handleAiAnalysis("VALUATION")}>估值分析</Button>
        </Space>
      </div>
    ...
    <div style={{ display: "flex", flexDirection: "column", height: 500 }}>
      <div style={{ flex: 1, overflow: "auto", padding: 16, background: "#fafafa", borderRadius: 8, marginBottom: 16 }}>
        {data.messages.length === 0 ? (
          ...
          data.messages.map((m) => (
            <div key={m.id} style={{ display: "flex", justifyContent: m.fromRole === "ADMIN" ? "flex-end" : "flex-start", marginBottom: 12 }}>
              <div style={{ maxWidth: "70%", padding: "8px 12px", borderRadius: 8, background: m.fromRole === "ADMIN" ? "#1890ff" : "#fff", color: m.fromRole === "ADMIN" ? "#fff" : "#333", border: m.fromRole === "ADMIN" ? "none" : "1px solid #e8e8e8" }}>
                <div style={{ fontSize: 12, marginBottom: 4, opacity: 0.8 }}>{m.fromName} ({m.fromRole === "ADMIN" ? "管理员" : m.fromRole === "EXPERT" ? "专家" : "当事人"})</div>
                <div>{m.content}</div>
  ...
  const auditContent = (
    <Table
      dataSource={data.auditLogs} rowKey="id" pagination={{ pageSize: 15 }}
      columns={[
    ...
    <div>
      <Card style={{ marginBottom: 16 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Button onClick={() => router.push("/admin/cases")}>返回列表</Button>
        </div>
      </Card>

      <Card title="案件管理" style={{ marginBottom: 16 }}>
        <Space size="large" wrap>
          <div>
            <span style={{ marginRight: 8 }}>状态变更:</span>
            <Select style={{ width: 160 }} placeholder="选择目标状态" value={selectedStatus} onChange={setSelectedStatus}
              options={allowedNext.map((s) => ({ value: s, label: STATUS_MAP[s]?.label || s }))}
            />
            <Button type="primary" style={{ marginLeft: 8 }} disabled={!selectedStatus} loading={statusChanging} onClick={handleStatusChange}>更新状态</Button>
          </div>
          <div>
            <span style={{ marginRight: 8 }}>指派专家:</span>
            <Select style={{ width: 160 }} placeholder="选择专家" value={selectedExpert} onChange={setSelectedExpert}
              options={availableExperts.map((e) => ({ value: e.id, label: `${e.name} (${e.availability === "AVAILABLE" ? "可用" : e.availability === "BUSY" ? "忙碌" : "不可用"})` }))}
            />
            <Button type="primary" style={{ marginLeft: 8 }} disabled={!selectedExpert} loading={assigning} onClick={handleAssignExpert}>指派</Button>
          </div>
        </Space>
      </Card>
      ...
      <Card>
        <Tabs items={[
          ...
          { key: "audit", label: "审计日志", children: auditContent },
        ]} />
      </Card>
